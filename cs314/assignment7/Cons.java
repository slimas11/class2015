...
public static Cons findpath(Object item, Object cave) {
	if(consp(cave)){
		Cons cueva = (Cons)cave;
		if(consp(first(cueva))){
			Cons path1 = cons("first",findpath(item,first(cueva)));
			Cons path2= cons("rest",findpath(item,rest(cueva)));
			
			if(first(path1).equals(item)){
				return cons("first",list("done"));
			}
			if(!first(path2).equals(item)){
				return cons("rest",findpath(item, rest(cueva)));
			}
		}
	if(first(cueva).equals(item)){
			return cons("first",list("done"));
		}
		return cons("rest",findpath(item, rest(cueva)));
	}
